...
## Redis pub/sub 기능을 사용한 마이크로서비스 간 데이터 브로드캐스팅

- redis pub/sub
   - [redis gate 참고](http://redisgate.kr/redis/command/pubsub_intro.php)
   ...
   - Pub/Sub는 **클러스터 모드를 지원**합니다.
   - 테스트시 Kafka가 아닌 Redis pubsub을 선택한 이유
      - kafka는 동일 consumer group내의 consumer들은 모두 다른 partition을 바라봐야 함. 둘 이상의 consumer가 같은 partition을 바라볼 수 없음. ==> 모든 인스턴스가 메시지를 받아야 한다면, 인스턴스들을 다 다른 consumer group에 둬야 한다.
      - redis는 subscribe하고 있는 모든 인스턴스에 그냥 메시지를 전달한다.
      - kafka는 이벤트를 저장하지만, redis는 저장하지 않고 현재 구독 중인 인스턴스에 보낸다. 지식 배포 이벤트는 별도로 저장할 필요가 없다고 판단.

- 테스트 시나리오
   1. User service에서 /redispub API 호출
      - request body
       ```json
       {
           "intentName": "의도명1",
       ...
       ```
   2. User service가 redis의 `scenario-knowledge` topic에 시나리오 정보 publish
      - publish되는 데이터
       ```json
       {
           "learnId": "5453f50a-4530-437b-8607-eeed03ef9445", // UUID
           "intentName": "의도명1",
           "nodeName": "노드명11"
       }
       ```
   3. Order service가 redis의 `scenario-knowledge` topic 리스닝하고 있다가, onMessage 이벤트 발생 시 topic subscribe

- User service
   - Publisher 쪽의 Redis config
       ```java
       @Configuration
   ...
   - Redis publish service
      - 컨트롤러에서  `scenarioDto.setLearnId(UUID.randomUUID().toString());` 추가하여 redis publish service 호출
       ```java
       @Slf4j
       @Service
       @RequiredArgsConstructor
...
- Order service
   ...
   - subscribe 코드
       ...
       ```
- 테스트
   - order service 3대 기동: 모두 동시에 subscribe 로그 확인됨.
   - order service 5대 기동: 모두 동시에 subscribe 로그 확인됨.
      - redis-cli에 `PUBSUB NUMSUB scenario-knowledge` 시 subscriber 5개 확인됨.
          ```
          > PUBSUB NUMSUB scenario-knowledge
          scenario-knowledge
          5
          ```
   - order service 5대 기동 중 한 대 죽음
      - redis-cli에 `PUBSUB NUMSUB scenario-knowledge` 시 subscriber 4개 확인됨.
          ```
          > PUBSUB NUMSUB scenario-knowledge
          scenario-knowledge
          4
          ```
      - 죽은 서버 다시 살려도 이미 지나간 이벤트는 받아올 수 없음
   - publish 시점에 기동된 order service가 없는 경우
      - redis-cli에 `PUBSUB NUMSUB scenario-knowledge` 시 subscriber 0개 확인됨.
          ```
          > PUBSUB NUMSUB scenario-knowledge
          scenario-knowledge
          0
...
- h2 (별도 설치)
   - 최신 h2는 자동 DB 생성이 어려우므로, 1.4대 버전으로 설치 권장
- kafka (별도 설치)
   - kafka server (kafka 2.13 설치)
      - 주의: h2와 9092 포트가 겹치므로, 실행 포트 변경 필요.
      - `config/server.properties` 파일 내 `listeners=PLAINTEXT://:9092`를 `listeners=PLAINTEXT://:9093`로 변경
   - zookeeper (kafka 설치 시 같이 설치됨)
   - kafka connector (confluent-6.1.10 설치)
   - mariadb-java-client-{version}.jar 파일을 `confluent 홈/share/java/kafka` 하위에 복사
- rabbitmq (별도 설치)
...
0. 위 소프트웨어 모두 기동
   ...
   - rabbitmq
      - 설치 시 자동 실행됨
      - 15672 포트로 접속 시 rabbitmq management 페이지가 뜨지 않는다면 `rabbitmq-server` 실행
1. discovery service
