...
2. [Configuration Service](http://localhost:8888/ecommerce/default)
   ...
   - `spring-cloud-starter-bootstrap` 라이브러리를 추가하여, application.yml 이전에 설정 정보를 넣을 수 있는 bootstrap.yml을 추가
      ...
      - user service의 bootstrap.yml: configuration service 서버 정보와 사용할 설정 파일의 이름을 bootstrap에 기재
     ```
     spring:
       cloud:
         config:
     ...
     ```
   - user-service.yml 설정파일 예시: '{cipher}암호화된 값' 추가하면 configuration service가 자동 복호화 수행 (암호화 key는 configuration service의 bootstrap.yml에 기재)
       ```
       spring:
         datasource:
